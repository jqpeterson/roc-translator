...
rxProtocol2Block ::  RocConfig -> Word8 -> PointTypes () -> Word8 -> Word8 -> IO BS.ByteString
rxProtocol2Block cfg pn ptid pc sp = do
  let port = rocConfigPort cfg
      commRate = rocCommSpeed cfg
      pt = decodePTID ptid
      firstpc = divTwo pc 
      secondpc = (pc - (divTwo pc))
      secondsp = firstpc
  s <- openSerial port defaultSerialSettings { commSpeed = commRate }
  _ <- send s $ BS.append (opCode167 pt pn firstpc sp cfg) (lzyBSto16BScrc.pack8to16 $ BS.unpack $ opCode167 pt pn firstpc sp cfg)   
  receivebs <- recvAllBytes s 255
  _ <- send s $ BS.append (opCode167 pt pn secondpc secondsp cfg) (lzyBSto16BScrc.pack8to16 $ BS.unpack $ opCode167 pt pn secondpc secondsp cfg)   
  receivebs1 <- recvAllBytes s 255
  let databytes = BS.append (BS.drop 10 $ BS.init $ BS.init receivebs)(BS.drop 10 $ BS.init $ BS.init receivebs1)
  return databytes
...
fbUnit107PT46 = FDPT 107 pt46 93 0 defaultRxProtocol
fbUnit107PT47 :: FB107PT47 
fbUnit107PT47 = FDPT 107 pt47 74 0 defaultRxProtocol
...
fbUnit107PT80 = FDPT 107 pt80 122 0 defaultRxProtocol
fbUnit107PT81 :: FB107PT81 
fbUnit107PT81 = FDPT 107 pt81 13 0 defaultRxProtocol
fbUnit107PT85 :: FB107PT85 
fbUnit107PT85 = FDPT 107 pt85 255 0 defaultRxProtocol
